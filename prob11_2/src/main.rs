...
impl SeatGrid {
    ...
    fn get_pos(&self, x: usize, y: usize) -> Option<Seat> {if x < self.grid[0].len() && y < self.grid.len() {Some(self.grid[y][x])}
        else {
            None
        }
    }
    ...
    fn get_neighbors(&self, x: usize, y: usize) -> i32 {
        ...
        'outer: for (x_off, y_off) in offsets {
            let mut neigh_x = x as i32 + x_off;
            let mut neigh_y = y as i32 + y_off;
            loop {
                if neigh_x >= 0 && neigh_y >= 0 {
                    let seat = self.get_pos(neigh_x as usize, neigh_y as usize);
                    match seat {
                        Some(seat) => {
                            match seat {
                                Seat::Floor => {
                                    neigh_x = neigh_x + x_off;
                                    neigh_y = neigh_y + y_off;
                                },
                                Seat::Empty => {
                                    continue 'outer;
                                },
